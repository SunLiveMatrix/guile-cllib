...
__gshared static short[] TERRAIN_INIT_DATA = [ // @suppress(dscanner.unnecessary.duplicate_attribute)
    //                                      V
...
__gshared static short[] TERRAIN_SCALE_DATA = [ // @suppress(dscanner.unnecessary.duplicate_attribute)
    //                                      V
...
void initWorldTerrain(World world, int terrSizeBits = 10, int x0 = 0, int z0 = 0) {
    ...
    terr.generateWithScale(123456, TERRAIN_INIT_DATA, terrSizeBits - 4, scaleterr); // init grid is 16x16 (1 << (9-7)) // @suppress(dscanner.style.number_literals)
    //terr.filter(1);
...
void makeCastleWall(World world, Vector3d start, Vector3d direction, int height, int length, int width, cell_t material) { // @suppress(dscanner.style.long_line)
...
void makeCastleWalls(World world, Vector3d start, int size, int height, int width, cell_t material) {
    world.makeCastleWall(Vector3d(start.x - size - width/2, start.y, start.z - size), Vector3d(1, 0, 0), height, size * 2 + width, width, material); // @suppress(dscanner.style.long_line)
    world.makeCastleWall(Vector3d(start.x - size, start.y, start.z - size - width/2), Vector3d(0, 0, 1), height, size * 2 + width, width, material); // @suppress(dscanner.style.long_line)
    world.makeCastleWall(Vector3d(start.x + size, start.y, start.z - size - width/2), Vector3d(0, 0, 1), height, size * 2 + width, width, material); // @suppress(dscanner.style.long_line)
    world.makeCastleWall(Vector3d(start.x - size - width/2, start.y, start.z + size), Vector3d(1, 0, 0), height, size * 2 + width, width, material); // @suppress(dscanner.style.long_line)
}
...
void prepareCastleBase(World world, Vector3d start, int size) {
    // basement - bedrock
    world.setCellRange(Vector3d(start.x - size - 3, start.y - 10, start.z - size - 3), Vector3d(size*2 + 6, 10, size*2 + 6), BlockId.bedrock); // @suppress(dscanner.style.long_line)
    world.setCellRange(Vector3d(start.x - size - 4, start.y - 11, start.z - size - 4), Vector3d(size*2 + 8, 10, size*2 + 8), BlockId.bedrock); // @suppress(dscanner.style.long_line)
    // empty
    world.setCellRange(Vector3d(start.x - size - 5, start.y, start.z - size - 5), Vector3d(size*2 + 10, 10, size*2 + 10), BlockId.air); // @suppress(dscanner.style.long_line)
    world.setCellRange(Vector3d(start.x - size - 6, start.y + 1, start.z - size - 6), Vector3d(size*2 + 12, 10, size*2 + 12), BlockId.air); // @suppress(dscanner.style.long_line)
    world.setCellRange(Vector3d(start.x - size - 7, start.y + 2, start.z - size - 7), Vector3d(size*2 + 14, 10, size*2 + 14), BlockId.air); // @suppress(dscanner.style.long_line)
    // floor
    world.setCellRange(Vector3d(start.x - size, start.y - 10, start.z - size), Vector3d(size*2, 10, size*2), BlockId.red_sand);//cobblestone // @suppress(dscanner.style.long_line)
}
...
void makeCastle(World world, Vector3d start, int size, int height) {
    ...
    world.setCellRange(Vector3d(start.x - 3, start.y, start.z - size - 4), Vector3d(7, height * 7 / 10, 7), BlockId.air); // @suppress(dscanner.style.long_line)
    world.setCellRange(Vector3d(start.x - 4, start.y, start.z - size), Vector3d(9, height * 7 / 10 + 1, 1), BlockId.air); // @suppress(dscanner.style.long_line)
    // corner towers
    ...
    world.setCellRange(Vector3d(start.x - 2, start.y, start.z - size - 4), Vector3d(5, height * 6 / 10, size), BlockId.air); // @suppress(dscanner.style.long_line)
}