        ...
        for (int y = 0; y < 8; y++) {
            for (int z = 0; z < 8; z++) {
                for (int x = 0; x < 8; x++) {
                    ...
                    if (visibleFaces) {
                        visitor.visit(world, world.camPosition, Vector3d(_pos.x + x, _pos.y + y, _pos.z + z), cells[index], visibleFaces); // @suppress(dscanner.style.long_line)
                    }
                    index++;
                }
            }
                ...
                for (int z = 0; z < 8; z++) {
                    for (int x = 0; x < 8; x++) {
                        ...
                        if (visibleFaces) {
                            ...
                            if (!_minerMesh) {
                                _minerMesh = new Mesh(VertexFormat(VertexElementType.POSITION, VertexElementType.NORMAL, VertexElementType.COLOR, VertexElementType.TEXCOORD0)); // @suppress(dscanner.style.long_line)
                            }

                            BlockDef def = BLOCK_DEFS[cells[index]];
                            def.createFaces(world, world.camPosition, Vector3d(_pos.x + x, _pos.y + y, _pos.z + z), visibleFaces, _minerMesh); // @suppress(dscanner.style.long_line)
                        }
                        index++;
                    }
                }
...
ulong flipBitsLeft(ulong n) {
    n &=  0xFFL; //
    return ((n&1) | ((n&2) << 7) | ((n&4) << 14) | ((n&8) << 21) | ((n&16) << 28) | ((n&32) << 35) | ((n&64) << 42) | ((n&128)<< 49)) & 0x0101010101010101L; // @suppress(dscanner.style.long_line)
}
    ...
    struct VisibilityCheckChunk {
        ...
        void tracePaths() {
            for (Dir dirFrom = Dir.min; dirFrom <= Dir.max; dirFrom++) {
                if ((visitedFromDirMask & (1 << dirFrom))) // @suppress(dscanner.suspicious.redundant_parens)
                    traceFrom(dirFrom);
            }
        }
    }
